...
model.xyBound = Var(within=NonNegativeReals)

for i in range(circles):
    model.add_component('x'+str(i),Var(within = NonNegativeReals,initialize=i))
    model.add_component('y'+str(i),Var(within = NonNegativeReals,initialize=i+1))

    model.add_component('Uboundx'+str(i), Constraint(expr=model.component('x'+str(i)) + circRad <= model.xyBound))
    model.add_component('Uboundy'+str(i), Constraint(expr=model.component('y'+str(i)) + circRad <= model.xyBound))

    model.add_component('Lboundx'+str(i), Constraint(expr=model.component('x'+str(i)) >= circRad ))
    model.add_component('Lboundy'+str(i), Constraint(expr=model.component('y'+str(i)) >= circRad ))
...
for i in range(circles):
    for j in range(i+1,circles,1):
        ...
        (model.component('y'+str(i)) - model.component('y'+str(j)))**2 >= (2*circRad)**2))
